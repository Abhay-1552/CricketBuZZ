    ...
    def cricket_score(self):
        try:
            ...
            for score in main_content:
                ...
                _team_content = []  # Initialize _team_content list for each match
                for _name in _team:
                    _team_flag = _name.find('img').get('src') if _name else 'N/A'
                    _team_name = _name.find('p').get_text() if _name else 'N/A'

                    span_elements = _name.find_all('span', class_='ds-text-compact-xs ds-mr-0.5')
                    for span in span_elements:
                        print(span.get_text() if span else '0/0')
